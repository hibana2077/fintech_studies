...
LastEditTime: 2023-12-22 00:42:19
...
# Label
# Rule: if in next 50 days, the price is higher than current price by 1% , then label is 1 else 0
for i in range(len(raw_data) - 50):
    current_price = raw_data.iloc[i]['close']
    future_max_price = np.max(raw_data.iloc[i+1:i+50]['close'].values)
    if future_max_price >= current_price * 1.002:
        raw_data.loc[raw_data.index[i], 'label'] = 1
    else:
        raw_data.loc[raw_data.index[i], 'label'] = 0
